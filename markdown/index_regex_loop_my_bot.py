...
for file_name_txt in os.listdir("."):
    if file_name_txt.endswith(".txt"):
        ...
        file_number_pattern = r"Numerical File ID: (\d+)"
        ...
        for file_name, file_number in zip(file_names, file_numbers):
            markdown_link = f"[{file_name.strip()}](https://t.me/PronVdoArchBot?start={file_number})"

            markdown_links+=f"{markdown_link}\n"

        file_name_txt = file_name_txt.replace(".txt","")

        with open(f"{file_name_txt}.md","w") as f:
